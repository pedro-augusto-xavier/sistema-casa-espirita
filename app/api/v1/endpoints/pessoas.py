...
def criar_pessoa(db: SessaoDB, dados: PessoaCreate):
...
def editar_pessoa(db: SessaoDB, pessoa_id: int, dados: PessoaUpdate):
    pessoa = crud.get(db, pessoa_id)
    if pessoa is None:
        raise HTTPException(status_code=404, detail="Pessoa não encontrada")
...
def desativar_pessoa(db: SessaoDB, pessoa_id: int):
    pessoa = crud.get(db, pessoa_id)
    if pessoa is None:
        raise HTTPException(status_code=404, detail="Pessoa não encontrada")
...
def reativar_pessoa(db: SessaoDB, pessoa_id: int):
    pessoa = crud.get(db, pessoa_id)
    if pessoa is None:
        raise HTTPException(status_code=404, detail="Pessoa não encontrada")
